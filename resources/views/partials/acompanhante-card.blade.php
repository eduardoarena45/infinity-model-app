...
<div class="bg-white dark:bg-gray-800 rounded-lg shadow-lg overflow-hidden transform hover:-translate-y-1 transition-all duration-300
    ...
    <a href="{{ route('vitrine.show', $perfil->id) }}" class="block">
        <div class="relative">
            {{-- CORRIGIDO: Usa o accessor 'foto_principal_url' do Model --}}
            <img src="{{ $perfil->foto_principal_url }}" alt="Foto de {{ $perfil->nome_artistico }}" class="w-full h-80 object-cover">
        ...
        <div class="p-4">
            ...
            {{-- CORRIGIDO: Usa a relação para pegar o nome da cidade --}}
            <p class="text-sm text-gray-500 dark:text-gray-400">{{ $perfil->cidade->nome ?? 'Cidade não informada' }}</p>
            ...
            <p class="text-lg font-bold text-green-600 dark:text-green-400 mt-4">R$ {{ number_format($perfil->valor_hora, 2, ',', '.') }} / hora</p>
        </div>
    </a>
</div>
