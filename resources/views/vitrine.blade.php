...
@section('content')
    ...
    <div class="container mx-auto p-4 py-12">
        ...
        @if($destaques->isNotEmpty())
            <div class="mb-16">
                <h2 class="text-2xl font-bold text-center text-gray-900 dark:text-white mb-8 border-b-2 border-pink-500 pb-2 inline-block">✨ Destaques</h2>
                <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
                    @foreach ($destaques as $perfil)
                        <a href="{{ route('vitrine.show', $perfil->id) }}" class="bg-white dark:bg-gray-800 rounded-lg shadow-lg overflow-hidden transform hover:scale-105 transition-transform duration-300 border-2 border-pink-500">
                            <div class="relative">
                                <img src="{{ Storage::url($perfil->imagem_principal_url) }}" alt="Foto de {{ $perfil->nome_artistico }}" class="w-full h-80 object-cover">
                                <div class="absolute top-2 right-2 bg-pink-600 text-white text-xs font-bold px-2 py-1 rounded-full">DESTAQUE</div>
                            </div>
                            <div class="p-4">
                                <div class="flex items-center justify-between">
                                    <h2 class="text-xl font-semibold text-gray-900 dark:text-white truncate">{{ $perfil->nome_artistico }}</h2>
                                    @if($perfil->is_verified) <span title="Perfil Verificado"><svg class="w-5 h-5 text-blue-500 flex-shrink-0" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor"><path fill-rule="evenodd" d="M6.267 3.455a3.066 3.066 0 001.745-.723 3.066 3.066 0 013.976 0 3.066 3.066 0 001.745.723 3.066 3.066 0 012.812 2.812c.051.643.304 1.254.723 1.745a3.066 3.066 0 010 3.976 3.066 3.066 0 00-.723 1.745 3.066 3.066 0 01-2.812 2.812 3.066 3.066 0 00-1.745.723 3.066 3.066 0 01-3.976 0 3.066 3.066 0 00-1.745-.723 3.066 3.066 0 01-2.812-2.812 3.066 3.066 0 00-.723-1.745 3.066 3.066 0 010-3.976 3.066 3.066 0 00.723-1.745 3.066 3.066 0 012.812-2.812zm7.44 5.252a.75.75 0 00-1.06-1.06L9 10.94l-1.72-1.72a.75.75 0 00-1.06 1.06l2.25 2.25a.75.75 0 001.06 0l3.75-3.75z" clip-rule="evenodd" /></svg></span> @endif
                                </div>
        ...
        <h2 class="text-2xl font-bold text-center text-gray-900 dark:text-white mb-8">Outros Perfis</h2>
        <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
            @forelse ($acompanhantes as $perfil)
                @if($perfil->nome_artistico)
                {{-- CÓDIGO DO CARD NORMAL RESTAURADO --}}
