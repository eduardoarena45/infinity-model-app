...
<?php endif; ?>
                <?php if(session('status') === 'gallery-updated'): ?>
                    <p x-data="{ show: true }" x-show="show" x-transition x-init="setTimeout(() => show = false, 3000)" class="text-sm text-gray-600 dark:text-gray-400"><?php echo e(session('success_message')); ?></p>
                <?php endif; ?>
            </div>
        </form>
    ...
    <?php else: ?>
        ...
        <div class="p-4 mt-6 text-sm text-yellow-800 rounded-lg bg-yellow-50 dark:bg-gray-800 dark:text-yellow-300" role="alert">
            <span class="font-medium">Limite de fotos atingido!</span> Você já tem <?php echo e($photo_count); ?> de <?php echo e($photo_limit); ?> fotos permitidas. Para adicionar mais, futuramente você poderá fazer um upgrade de plano.
        </div>
    ...
    <div>
        ...
        <?php if($media && $media->count() > 0): ?>
            <div class="mt-4 grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-4">
                <?php $__currentLoopData = $media; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $foto): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
